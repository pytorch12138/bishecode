...
model.load_state_dict(torch.load( './model/hrnet96/model_96_208.pth')['state'])
model.eval()
...
#      print(miou[0])
#      with open('./result/hrent32.txt','a',encoding='utf-8') as f:
...
print(model)
# model.load_state_dict(torch.load( './model/river_hrent_48/model_96_204.pth',)['state'])
# model.eval()
# simoid=nn.Sigmoid()
# softmax = torch.nn.Softmax(dim=1)
# img=cv2.imread('./river/imgs/332.png')
# img1=img/255.0
# img_x=torch.from_numpy(img1).float()
# img_x=img_x.permute(2,0,1)
# img_x=torch.stack([img_x])
...
# output=output*255
# output = np.array(output, dtype=np.uint8)
# label=cv2.imread('./river/labels/332.png')
