...
def doISODATARGB(image, K: int, TN: int, TS: float, TC:int, L: int, I: int) -> Image.Image:
    ...
    newImg = numpy.zeros((imgX, imgY), dtype=numpy.uint8)
    i=0
    for cluster in clusterList:
        for pixel in cluster.pixelList:
            newImg[pixel.x, pixel.y]=i
        ...
        i+=1
    return Image.fromarray(newImgArray, mode = "RGB"),newImg,i

if __name__ == '__main__':
    image = Image.open('./6289.png')
    argvK=4
    argvTN=0.3 #每个类别中样本最小数目（期望）（默认为3）
    argvTS=30.0 #每个类别的最下样本数
    argvTC=20 #每个类别的最小距离
    argvL=2 #每次允许合并的最大类别对的数 量
    argvI=20 #每次迭代数目
    stime=time.time()
    result, newImg,i= doISODATARGB(image, argvK, argvTN, argvTS, argvTC, argvL, argvI)
    img_=np.array(newImg*255/i,np.uint8)
    output=newImg
    ...
    etime=time.time()
    print("迭代所用数目",etime-stime)
    plot.figure("Result")
    plot.imshow(result)
    cv2.imshow("img",img_)
    cv2.imshow("new_img",new_img)
    cv2.imwrite('./label_6289.png',new_img)
    plot.show()
    cv2.waitKey(0)